...
import { ArrowRight, Code2, Cpu, Users, Target } from "lucide-react";
...
export default function About() {
  return (
    <main className="min-h-screen pt-32 pb-24 px-6 lg:px-24 max-w-7xl mx-auto flex flex-col">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center mb-32">
        ...
        >
          ...
          <div className="space-y-6 text-slate-400 text-lg leading-relaxed font-medium">
            ...
            <p>
              My work focuses on bridging the gap between solid backend engineering and high-performance frontends. I partner closely with founders to architect scalable platforms, implement complex Stripe Connect transaction flows, and build dynamic, conversion-optimized interfaces using Next.js and Sharetribe Flex / GO.
            </p>
            <p>
              Beyond client architecture, I am deeply invested in the tech ecosystem. Based in Osun State, Nigeria, I serve as the Software Director of my university's tech community, coordinating developer teams and empowering the next generation of software engineers.
            </p>
          </div>
        </motion.div>

        {/* Image Placeholder (Apple-style asymmetrical framing) */}
        <motion.div 
        ...
        >
          <div className="aspect-[3/4] w-full rounded-3xl overflow-hidden bg-slate-800 border border-slate-700 relative shadow-2xl group">
            {/* Using the profile.png from your homepage */}
            <Image src="/your-professional-photo.jpg" alt="Oyebode Precious Isaac" fill className="object-cover object-top transition-transform duration-700 group-hover:scale-105" /> 
            
            {/* Subtle Yellow Accent overlay */}
            <div className="absolute inset-0 bg-gradient-to-t from-[#0F172A] via-[#0F172A]/20 to-transparent opacity-60"></div>
          </div>
          
          {/* Decorative Elements */}
          <div className="absolute -bottom-6 -left-6 w-24 h-24 bg-[#FACC15] rounded-full blur-3xl opacity-20 -z-10"></div>
        </motion.div>
      </section>
      ...
      >
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-1 bg-gradient-to-r from-transparent via-[#FACC15] to-transparent"></div>
...
// Reusable component for the philosophy cards
function PillarCard({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
  return (
    <div className="bg-slate-800/30 border border-slate-700/50 p-8 rounded-3xl hover:border-[#FACC15]/30 hover:bg-slate-800/50 transition-all group">
      <div className="text-[#FACC15] mb-6 p-4 bg-[#0F172A] rounded-2xl inline-block shadow-inner group-hover:scale-110 transition-transform">
        {icon}
      </div>
